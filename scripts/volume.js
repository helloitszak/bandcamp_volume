var desktop_player = document.getElementsByClassName("inline_player desktop-view")[0]
if (desktop_player)
{
	var audiotag = document.getElementsByTagName("audio")[0] 
	desktop_player.querySelector("tr:first-child td:first-child").setAttribute("rowspan", "3")
	...
	var playprogbar = desktop_player.querySelector(".progbar_empty")
	...
	range.step = 0.01
	range.min = 0
	range.value = audiotag.value

	chrome.storage.local.get("volume", function(items) {
		audiotag.volume = items["volume"]
		range.value = items["volume"]
	});
	...
	range.addEventListener("mouseup", function(evt) {
		chrome.storage.local.set({"volume":evt.target.value})
	});

	volcontainer.appendChild(input)
	volcontainer.appendChild(range)
	col.appendChild(volcontainer)
	desktop_player.querySelector("tbody").appendChild(row)
}
